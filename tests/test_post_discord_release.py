...
class SplitSectionTests(unittest.TestCase):
    ...
    def test_every_real_changelog_section_can_be_posted(self):
        ...
        Deliberately does not require any section to be oversized. The 0.3.0
        section is 8213 characters on main but 1204 on dev, since the release
        branch folded the pending notes into it -- so asserting on size here
        would make the suite pass or fail depending on which branch it runs
        from. What matters is that whatever is there splits correctly.
        """
