...
Advanced Power Infrastructure uses Semantic Versioning in `MAJOR.MINOR.PATCH` form, with the additional pre-`1.0.0` rules below for gameplay and prototype changes.
...
- Adding a new power-generation or power-storage entity, recipe, technology, or compatibility path.
- Changing progression numbers such as power output, efficiency, storage capacity, recipe cost, prerequisites, or unlock timing.
...
- Release zip files use `{mod-name}_{version}.zip`, for example `advanced-power-infrastructure_0.1.0.zip`.
