...
```text
  Major Features:
    ...
    - Made distance cost temperature. Heat now cools as it travels along a pipe, and cools faster the harder the run is working, so a reactor block can no longer feed an arbitrarily long arm at full strength. Useful reach also shrinks as tiers rise: roughly 50 tiles at the steel tier down to 18 at mk4, before load is taken into account.
