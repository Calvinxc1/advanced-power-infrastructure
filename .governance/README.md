...
Kind branches may add `.governance/templates/` for branch-specific local setup shapes. On the Factorio modding branch, `templates/factorio-local-workflow.yaml` scaffolds the local Factorio workflow/status file that each mod workspace can copy under `.governance/local/` and load through `.governance/local/index.yaml`.
