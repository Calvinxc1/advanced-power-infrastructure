...
- Confirm `.governance/local/status.yaml` exists when the workspace participates in local governance status.
...
- Confirm downstream workspaces rely only on `.governance/...` paths for active governance.
- Confirm no top-level governance tree exists outside `.governance/`.
