...
```text
.governance/templates/governance-bootstrap-prompt.md
```
...
Use `.governance/governance-init.md` as the setup guide and `.governance/templates/local-status.yaml` as the local status shape.
...
5. Ensure `.governance/local/index.yaml` exists, using `.governance/templates/local-index.yaml` as the loose starter shape when needed. Do not prescribe the rest of `.governance/local/`.
6. Create or update `.governance/local/status.yaml` from `.governance/templates/local-status.yaml`.
