...
After the target policy line and agent kind are confirmed, select the current mapped `/main` endpoint branch for that policy line and kind, then seed or update the downstream workspace from that selected branch's `.governance/` tree.
