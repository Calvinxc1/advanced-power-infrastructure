...
Transition downstream workspaces from the selected target branch's `.governance/` tree. Preserve workspace-local governance and create or update downstream status at `.governance/local/status.yaml` from `.governance/templates/local-status.yaml`.
...
4. Copy or merge the target branch's `.governance/` product into this workspace, preserving workspace-local governance.
...
7. Create or update this workspace's `.governance/local/status.yaml` from `.governance/templates/local-status.yaml` so it records the target branch, target active canon version, target generalized canon version, source repository, source branch, source commit when known, verification date, and any unresolved reconciliation notes.
